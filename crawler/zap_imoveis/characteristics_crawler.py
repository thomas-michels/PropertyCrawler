...
def start_characteristics_crawler(message: EventSchema) -> RawProperty:
    try:
        ...
        raw_property = RawProperty(
            ...
            price=float(data["listing"]["pricingInfos"][0].get("price", 0)),
            description=data["listing"]["description"],
            neighborhood=data["listing"]["address"]["neighborhood"],
            rooms=int(data["listing"]["bedrooms"][0]),
            bathrooms=int(data["listing"]["bathrooms"][0]),
            size=float(data["listing"]["usableAreas"][0]),
            parking_space=int(data["listing"]["parkingSpaces"][0]),
            modality=modality,
            property_url=url,
            image_url=image_url,
            type=property_type,
            ...
            zip_code=data["listing"]["address"]["zipCode"],
